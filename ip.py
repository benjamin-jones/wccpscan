...
class Packet:
    ...
    def _assemble(self, cksum):
        ...
        header =  struct.pack('!BBHHHBB',
                              ...
                              self.p & 0xff)
        if cksum:
            self.sum = inetutils.cksum(header + b'\000\000' + self.__src +
                                  self.__dst)
            packet = header + struct.pack('!H', self.sum) \
                     + self.__src + self.__dst
        else:
            packet = header + b'\000\000' + self.__src + self.__dst 
    ...
    def _disassemble(self, packet, cksum):
